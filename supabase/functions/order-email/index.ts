...
// Admin-only notification on new order INSERT.
// Dispensary confirmation (with PDF invoice, OMMA license, batch/Metrc info)
// is handled by the Netlify wholesale-place-order function.
Deno.serve(async (req) => {
  try {
    ...
    const [{ data: items }, { data: dispensary }] = await Promise.all([
      supabase.from("wholesale_order_items").select("*").eq("order_id", order.id),
      supabase.from("dispensaries").select("*").eq("id", order.dispensary_id).single(),
    ]);
    ...
    console.log(`Admin notification sent for #${shortId}`);
